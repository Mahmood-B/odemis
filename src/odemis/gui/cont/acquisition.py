...
class CryoAcquiController(object):
    ...
    def __init__(self, tab_data, panel, tab):
        ...
        self._panel.param_Zmin.Bind(wx.EVT_CHAR, self._on_filter_Zmin_param)
        self._panel.param_Zmax.Bind(wx.EVT_TEXT_ENTER, self._on_Zmax_param)
        self._panel.param_Zmax.Bind(wx.EVT_CHAR, self._on_filter_Zmax_param)
        self._panel.param_Zstep.Bind(wx.EVT_TEXT_ENTER, self._on_Zstep_param)
        self._panel.param_Zstep.Bind(wx.EVT_CHAR, self._on_filter_Zstep_param)
    ...
    def _on_filter_Zmin_param(self, event):
        cursor= self._panel.param_Zmin.GetInsertionPoint()
        value=event.GetEventObject().GetValue()
        char=chr(event.GetKeyCode())
        freq=Counter(value)
        if cursor in [len(value),len(value)-1,len(value)-2]: # if cursor is in place 2nd, or any of the last 3 places
            if char in ["ļ", "ĺ"]: # if left or right arrow 
                 event.Skip()
            else:
                return
        elif cursor==0: # if cursor is at the beginning 
            if char in ["-","ļ"] or char.isdigit():
                if char=="-" and freq["-"]==1:
                    return
                elif char.isdigit():
                    return
            ...
            if cursor==1 and char=="\x08":
                return
            else:
                if char.isdigit() or char in ["\x08","ļ", "ĺ", "."]: # if digit, backspace, right, left or dot  
                    if char=="." and freq["."]==1:
                        return
                    else:
                        event.Skip()
    ...
    def _on_Zmin_param(self, _):
        ...
        """
        input = list(self._panel.param_Zmin.GetValue())
        input = [char for char in input if char != "µ" and char != "m" and char != " "]
        raw_input = "".join(input)
        if (
            raw_input.islower() or raw_input.isupper()
        ):  # has at least one alphabetical character
            # if so, set the default value
            self._panel.param_Zmin.SetValue("-10 µm")

        else:  # doesn't have any alphabetical
            for char in raw_input:
                if not char.isdigit() and char not in ["-", "."]:
                    self._panel.param_Zmin.SetValue("-10 µm")
                    return
            freq = Counter(raw_input)
            if freq["-"] > 1 or freq["."] > 1:
                self._panel.param_Zmin.SetValue("-10 µm")
                return
            if raw_input.find("-") != 0 or raw_input.find(".") == 0:
                self._panel.param_Zmin.SetValue("-10 µm")
                return
            else:
                if float(raw_input) < 0 and float(raw_input) >= -1000:
                    # TODO round the float to 2 precision
                    result = (
                        int(float(raw_input))
                        if float(raw_input) == int(float(raw_input))
                        else float(raw_input)
                    )
                    self._panel.param_Zmin.SetValue(str(result) + " µm")
                    self._tab_data.zMin.value = result * 1e-6
                else:
                    self._panel.param_Zmin.SetValue("-10 µm")

    def _on_filter_Zmax_param(self, event):
        cursor= self._panel.param_Zmax.GetInsertionPoint()
        value=event.GetEventObject().GetValue()
        char=chr(event.GetKeyCode())
        freq=Counter(value)
        if cursor in [len(value),len(value)-1,len(value)-2]: # if cursor is in place 2nd, or any of the last 3 places
    ...
    def _on_Zmax_param(self, _):
        ...
        the Zmax parameter
        """
        input = list(self._panel.param_Zmax.GetValue())
        input = [char for char in input if char != "µ" and char != "m" and char != " "]
        raw_input = "".join(input)
        if (
            raw_input.islower() or raw_input.isupper()
            ...
            if freq["+"] > 1 or freq["."] > 1:
                self._panel.param_Zmax.SetValue("+10 µm")
                return
            if (raw_input.find("+") != -1 and raw_input.find("+") != 0) or raw_input.find(
                "."
            ) == 0:
                self._panel.param_Zmax.SetValue("+10 µm")
                return
                ...
                if float(raw_input) > 0 and float(raw_input) <= 1000:
                    ...
                    self._panel.param_Zmax.SetValue("+" + str(result) + " µm")
                    self._tab_data.zMax.value = result * 1e-6
                else:
                    self._panel.param_Zmax.SetValue("+10 µm")

    def _on_filter_Zstep_param(self, event):
        cursor= self._panel.param_Zstep.GetInsertionPoint()
        value=event.GetEventObject().GetValue()
        char=chr(event.GetKeyCode())
        freq=Counter(value)
        if cursor in [len(value),len(value)-1,len(value)-2]: # if cursor is in any of the last 3 places
            if char in ["ļ", "ĺ"]: # if left or right arrow 
                 event.Skip()
            else:
                return
            ...
            if char in ["+","-","ļ"] or char.isdigit():
                if (char=="-" and (freq["-"]==1 or freq["+"]==1)) or (char=="+" and (freq["+"]==1 or freq["-"])):
                    return
                elif char.isdigit() and (freq["-"]==1 or freq["+"]==1):
                    return
                else:
                    event.Skip()
            else:
            ...
            # TODO/FIXME allow dot 
            if char.isdigit() or char in ["\x08","ļ", "ĺ"]: # if digit, backspace, right or left arrow 
                event.Skip()
            else: # if non-digit 
                return
    ...
    def _on_Zstep_param(self, _):
        ...
        """
        input = list(self._panel.param_Zstep.GetValue())
        input = [char for char in input if char != "µ" and char != "m" and char != " "]
        raw_input = "".join(input)
        if (
            raw_input.islower() or raw_input.isupper()
            ...
            for char in raw_input:
                if not char.isdigit() and char not in ["+", "-", "."]:
                    ...
                    return
            freq = Counter(raw_input)
            if freq["+"] > 1 or freq["."] > 1 or freq["-"] > 1:
                self._panel.param_Zstep.SetValue("1 µm")
                return
            if (
                (raw_input.find("+") != -1 and raw_input.find("+") != 0)
                or (raw_input.find("-") != -1 and raw_input.find("-") != 0)
                or (raw_input.find("+") != -1 and raw_input.find(".") == 0)
            ):
                self._panel.param_Zstep.SetValue("1 µm")
                return
            else:
                if float(raw_input) >= -100 and float(raw_input) <= 100:
                    # TODO round the float to 2 precision
                    result = (
                        int(float(raw_input))
                        if float(raw_input) == int(float(raw_input))
                    ...
                    self._tab_data.zStep.value = result * 1e-6
                    if result > 0:
                        self._panel.param_Zstep.SetValue("+" + str(result) + " µm")
                    elif result < 0:
                        self._panel.param_Zstep.SetValue(str(result) + " µm")
                else:
                    self._panel.param_Zstep.SetValue("1 µm")
