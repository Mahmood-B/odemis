...
from concurrent.futures._base import RUNNING, CANCELLED, FINISHED
...
class ZStackAcquisitionTask(object):
    ...
    def __init__(self, future, streams, zlevels, settings_obs):
        ...
        self._zpos = (
            next(iter(zlevels.values()))
            if len(next(iter(zlevels.values()))) > 1
            else None
        )
        self._zstack_acqui = True if len(next(iter(zlevels.values()))) > 1 else False
    ...
    def cancel(self, _):
        ...
        """
        if self._future_state == FINISHED:
            return False
    ...
    def _estimate_zstep_duration(self):
        ...
        zstep = 0
        if self._zstack_acqui:
            zstep = abs(self._zpos[0] - self._zpos[1])
        return estimateMoveDuration(abs(zstep), speed, 0.01)
    ...
    def estimate_total_duration(self):
        ...
        for s in self._streams:
            ...
            if isinstance(s, FluoStream) and self._zstack_acqui:
                nr_FM_streams += 1
        if self._zstack_acqui:
    ...
    def _construct_zcube(self, data_arrays):
        """
        Upgrades the metadata of the acquired data to a metadata of 3D acquired data
        ...
        pz = (
            (self._zpos[-1] - self._zpos[0]) / (len(self._zpos) - 1)
            if self._zstack_acqui
            else 1e-6
        )
        if pz < 0:
    ...
    def run(self):
        ...
        for stream in self._streams:
            ...
            for i, z in enumerate(self._zlevels[stream]):
                ...
                if isinstance(stream, FluoStream) and self._zstack_acqui:
            ...
            if self._zstack_acqui and isinstance(stream, FluoStream):
